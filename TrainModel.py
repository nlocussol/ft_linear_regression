    ...
    try:
        with open('theta.csv', 'w', newline='') as file:
            ...
            for _ in range(1000):
                ...
                writer.writerow([theta0 * np.linalg.norm(y), theta1 * np.linalg.norm(X)])


            theta0 = theta0 * np.linalg.norm(y)
            theta1 = theta1 * (np.linalg.norm(y) / np.linalg.norm(X))

            writer.writerow([theta0, theta1])
            
            linear_reg = theta0 + theta1 * X
